...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_student(
			...
			ensure!(!Students::<T>::contains_key(&student), Error::<T>::StudentExisted);

			let student_info = Student { name, age, grade };
		...
		pub fn update_student(origin: OriginFor<T>, age: u16, grade: u8) -> DispatchResult {
			...
			ensure!(Students::<T>::contains_key(&student), Error::<T>::NotFoundStudent);

			let mut student_info = <Students<T>>::get(&student);
